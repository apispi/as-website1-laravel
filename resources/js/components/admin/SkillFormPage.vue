<template>
  <AdminShell :user="user" :csrf-token="csrfToken" page="skills">
    ...
    <div class="page-header">
      <div>
        <a href="/admin/skills" class="back-link">← Skills</a>
        <h1 class="page-title">{{ skill ? 'Edit Skill' : 'New Skill' }}</h1>
      </div>
    </div>
    ...
    <div class="form-card">
      <form method="POST" :action="formAction">
        ...
        <input v-if="skill" type="hidden" name="_method" value="PUT">

        <div class="form-grid">
          <div class="form-group full">
            <label>Name <span class="req">*</span></label>
            <input type="text" name="name" :value="skill?.name" required placeholder="e.g. Document Analysis">
          </div>

          <div class="form-group">
            <label>Slug <span class="req">*</span></label>
            <input type="text" name="slug" :value="skill?.slug" required placeholder="e.g. document-analysis">
            <p class="hint">Unique identifier, lowercase with hyphens.</p>
          </div>

          <div class="form-group">
            <label>Category</label>
            <input type="text" name="category" :value="skill?.category" placeholder="e.g. Document Processing">
          </div>

          <div class="form-group full">
            <label>Description</label>
            <textarea name="description" rows="4" placeholder="Describe what this skill does…">{{ skill?.description }}</textarea>
          </div>

          <div class="form-group">
            <label>Sort Order</label>
            <input type="number" name="sort_order" :value="skill?.sort_order ?? 0" min="0">
          </div>

          <div class="form-group checkboxes">
            <label class="checkbox-label">
              <input type="hidden" name="is_active" value="0">
              <input type="checkbox" name="is_active" value="1" :checked="skill?.is_active ?? true">
        ...
        <div class="form-actions">
          <button type="submit" class="btn-submit">{{ skill ? 'Save Changes' : 'Create Skill' }}</button>
          <a href="/admin/skills" class="btn-cancel">Cancel</a>
        </div>
      </form>
    </div>
...
const formAction = props.skill ? `/admin/skills/${props.skill.id}` : '/admin/skills';
</script>
