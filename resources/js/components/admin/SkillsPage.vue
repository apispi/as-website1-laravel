<template>
  <AdminShell :user="user" :csrf-token="csrfToken" page="skills">
    ...
    <div class="toolbar">
      <div class="search-wrap">
        ...
        <input v-model="search" type="text" placeholder="Search skills…" class="search-input">
      </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="skill in filtered" :key="skill.id">
            <td>
              <a :href="`/admin/skills/${skill.id}/edit`" class="skill-cell">
            ...
            <td>
              <span class="status-badge" :class="skill.is_active ? 'active' : 'inactive'">
                {{ skill.is_active ? 'Active' : 'Inactive' }}
              </span>
            </td>
            <td class="actions">
              <a :href="`/admin/skills/${skill.id}/edit`" class="btn-ghost">Edit</a>
              <form method="POST" :action="`/admin/skills/${skill.id}`" style="display:inline;"
                    @submit.prevent="confirmDelete($event, skill.name)">
          ...
          <tr v-if="filtered.length === 0">
            <td colspan="4" style="text-align:center; padding:2.5rem; color:#4b5563;">No skills found</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const filtered = computed(() => {
  return props.skills.filter(s => {
    ...
    const matchesFilter =
      filter.value === 'All' ||
      (filter.value === 'Active'   && s.is_active) ||
      (filter.value === 'Inactive' && !s.is_active) ||
      s.category === filter.value;
...
.data-table tbody tr:hover .skill-name { color: #fca5a5; }
.data-table tbody tr:hover .muted { color: #9ca3af; }

.skill-cell { display: flex; align-items: center; gap: 0.75rem; text-decoration: none; }
.skill-cell:hover .skill-name { color: #fca5a5; }
.skill-icon { width: 34px; height: 34px; border-radius: 0.5rem; flex-shrink: 0; background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.2); display: flex; align-items: center; justify-content: center; font-size: 1rem; color: #fca5a5; }
.skill-name { font-size: 0.875rem; font-weight: 600; color: #e5e7eb; }
.skill-slug { font-size: 0.75rem; color: #4b5563; margin-top: 0.1rem; font-family: monospace; }
